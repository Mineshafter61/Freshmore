# 2_1_Understand Science from Atom to Molecule
...
- **Usually refers to the first ionisation energy** (IE₁): the least amount of energy required to remove a single electron from a neutral atom
- **Always positive** (endothermic reaction) (IE > 0 for all cases)
    - $M_{(g)} \to M^{+}+e^{-}$: 1st ionisation energy, **IE₁**
    - $M^{+}_{(g)} \to M^{2+}_{(g)}+e^{-}$: 2nd ionisation energy, **IE₂ > IE₁**
...
- Energy change accompanying the **addition** of 1 mol of electrons to 1 mol of gaseous atoms or ions
    - $X_{(g)} + e^{-}\to X_{(g)}^{-}$: 1st electron affinity, EA₁ < 0 generally
    - $X^{-}_{(g)} + e^{-}\to X_{(g)}^{2-}$: 2nd electron affinity, EA₂ > 0 always
- Considering EA₁, if the electron is easily attracted to the atom, EA₁ is **generally negative** (exothermic reaction)
- EA₂ and successive EAs are **always positive**
    - Energy is required (i.e. absorbed) to overcome electrostatic repulsion between anion X⁻ and electron e⁻
